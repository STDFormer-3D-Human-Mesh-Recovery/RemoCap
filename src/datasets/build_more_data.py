...
from src.datasets.human_mesh_tsv_more_data import MeshTSVYamlDataset
from src.datasets.hand_mesh_tsv import HandMeshTSVYamlDataset
# from torch.nn.utils.rnn import pad_sequence
# from src.datasets.thuman_ds import THumanDataset
...
def make_data_loader(args, yaml_file, is_distributed=True,
    ...
    #     pin_memory=True,
    #     drop_last=True
    #     # collate_fn=pad_sequence,  # 使用 pad_sequence 来进行 padding
    # )
    return data_loader


def make_thuman_loader(args, yaml_file, is_distributed=True,
        is_train=True, start_iter=0, scale_factor=1):

    dataset = THumanDataset(yaml_file, args, is_train=is_train, scale_factor=scale_factor)
    logger = logging.getLogger(__name__)
    if is_train==True:
        shuffle = True
        images_per_gpu = args.per_gpu_train_batch_size
